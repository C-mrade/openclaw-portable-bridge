...
func validCapabilities(v []string) bool {
	...
	allowed := map[string]bool{"system.info": true, "process.list": true, "process.start": true, "process.stop-owned": true, "shell.run": true, "files.list": true, "files.read": true, "files.write": true, "files.upload": true, "files.download": true, "session.disconnect": true}
